	...
	this.init = function(element, controllers, attrs) {
		...
		ngModelCtrl.$parsers.push(function(value) {
			if (value) {
				...
				console.log('da',value.format(dateModel));
				// return value.format(dateModel);
				return new Date(value.format(dateModel))
			}
			else {
				return null;
			}
...
.directive('liDateDropdown', [function() {
	'use strict';
	return {
		templateUrl: 'slds/views/fields/date/field-date-dropdown.html',
...
.directive('liDateYearPicker', ['DateService', function(DateService) {
	'use strict';
	return {
		templateUrl: 'slds/views/fields/date/field-date-yearpicker.html',
		link: function(scope, element, attrs, controllers) {
			var currentIndex = 0;
			var currentYear;
